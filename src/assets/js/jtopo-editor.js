...
TopologyEditor.prototype.initMenus = function () {
  ...
  self.nodeMenu.on('mouseover', function (event) {
    ...
    if (menuX + self.nodeTextPosMenu.width() * 2 >= self.stage.width) {
      menuX -= (self.nodeTextPosMenu.width() + self.nodeMenu.width()+7)
    } else {
      menuX +=7
    }
    if (text === '文字位置') {
      ...
      self.nodeTextPosMenu.css({
        top: parseInt(this.style.top) + $(document.getElementById('change-node-text-pos')).height()+34,
        left: menuX
      }).show()
    } else if (text === '应用布局') {
      ...
      if (event.target) {
        if (event.target instanceof JTopo.Node) { // 处理节点右键菜单事件
          ...
          if (selectedNodes && selectedNodes.length > 1) {
            ...
            // 判断边界出是否能完整显示弹出菜单
            if (menuX + self.nodeMenu.width()+30 >= self.stage.width) {
              menuX -= self.nodeMenu.width()-30
            } else {
              menuX += 30
            }
